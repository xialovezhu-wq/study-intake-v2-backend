...
The ordinary read binding is
`required_mcp_tool=mcp__kaoyan_english_read__english_read_bundle`. If that
tool snapshot is unavailable, fail closed with
`fallback_reason=tool_snapshot_missing`; do not substitute a different
subject reader or any cross-subject tool. Historical runtime snapshots may expose the same read-only
operation as `mcp__kaoyan_read__english_read_bundle`,
`mcp__kaoyan_read_v2__english_read_bundle`, or
`mcp__kaoyan_read_v3__english_read_bundle`. These legacy names are accepted
only for reopening their own immutable evidence.
